...
Download the Scope Java Agent, replacing `0.2.1` with the latest version.

```bash
cd /path/to/scope/agent
curl -s -o scope-agent.jar "https://repo1.maven.org/maven2/com/undefinedlabs/scope/scope-agent/0.2.1/scope-agent-0.2.1.jar"
```
